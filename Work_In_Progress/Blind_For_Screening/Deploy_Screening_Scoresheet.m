function [mdbf]=Deploy_DF_Scoresheet(prefill_out, acc_out, dcmdir_in)

%% preset parameters
side = 3;
knee = 'B';

template_dir =  'E:\MOST-Renewal-II\XR\BLINDING\Scoresheet_Templates\ScreeningPA_Templates';
...
mdbf = horzcat(out_dir,'MOST_XR_ScreeningPA_',dvd_date,'.mdb');
...
for ix=1:size(u_id,1)
    ...
    if(~isempty(tmpseracc))
        tmpacc = unique(tmpseracc);
        tmpacc = tmpacc{1};
    else
        tmpseracc = prefill_out(tmpjx,7);
        tmpseracc = regimatch(tmpseracc,'.{1,}');
        tmpacc = unique(tmpseracc);
        tmpacc = tmpacc{1};
    end
